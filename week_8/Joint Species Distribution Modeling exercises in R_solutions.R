...
# Call the Hmsc package (needs to be done every time you start a new R session)
library(Hmsc)
...
# we build the model object. Here we specify a square term for temperature,
# which then automatically also includes the linear term.
XFormula = ~ hab + poly(clim, degree = 2, raw = TRUE)
# OR
XFormula = ~ hab + clim^2 + clim
